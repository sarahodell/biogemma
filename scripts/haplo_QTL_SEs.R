...
h_high=c()
for(o in hapseq){
  v=se4[se4$haplotype==paste0("HAPGRP_",o),]$emmean
  low=se4[se4$haplotype==paste0("HAPGRP_",o),]$lower.CL
  high=se4[se4$haplotype==paste0("HAPGRP_",o),]$upper.CL
...
print(ggplot(fprop,aes(x=as.factor(hapgrp),y=h_value_adj,fill=founder_f)) +
 geom_bar(position="stack",stat="identity")+
  ...
  scale_fill_manual(values=colorcodes$hex_color[],labels=factor(colorcodes$founder,levels=founders))+
