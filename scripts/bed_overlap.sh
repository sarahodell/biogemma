...
if [ -f ibd_segments/comparison/c${chr}_germline_600K_RefinedIBD_overlap.bed ]; then rm ibd_segments/comparison/c${chr}_germline_600K_RefinedIBD_overlap.bed; fi
if [ -f ibd_segments/comparison/c${chr}_germline_WGS_RefinedIBD_overlap.bed ]; then rm ibd_segments/comparison/c${chr}_germline_WGS_RefinedIBD_overlap.bed; fi
...
echo "GERMLINE 600K vs. WGS RefinedIBD"

while read pair; do
  bedtools intersect -a ibd_segments/comparison/tmp/${pair}_600K_c${chr}.bed -b ibd_segments/comparison/tmp/${pair}_RefinedIBD_c${chr}.bed >> ibd_segments/comparison/c${chr}_germline_WGS_RefinedIBD_overlap.bed
done < ibd_segments/comparison/c${chr}_germline_WGS_RefinedIBD_pair_list.txt
