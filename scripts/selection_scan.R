...
size=dim(fprobs[[1]])[2]
fsums=sapply(seq(1,size), function(j) sapply(seq(1,16),function(i) sum(fprobs[[i]][,j])))

p_chi=sapply(seq(1,dim(fsums)[2]), function(x) chisq.test(x=round(fsums[,x]),p=null)$p.value)
#p_chi=t(p_chi)
p_chi=data.frame(p_chi=unlist(p_chi),stringsAsFactors=F)
...
png(sprintf('selection/bg%s_chi_squared_scan.png',chr),width=1600, height=800)
print(ggplot(p_chi,aes(x=pos/1e6,y=-log10(p_chi))) + geom_point() + geom_hline(yintercept=-log10(0.05/size),color="red") + xlab("Position (Mb)") + ylab("Chi-Squared -log10(P-Value)") + ggtitle(sprintf("Chi-Squared Test for Representation of Founder Alleles on Chromosome %s",chr)))
dev.off()
