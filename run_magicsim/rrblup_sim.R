...
us=fread('pgs/DTS_marker_effect_sizes.txt',data.table=F)
...
for(chr in 1:10){
  ...
  i=intersect(colnames(X),subu$marker)
  ms=colnames(X)
  u=subu[match(i,subu$marker),]$effect
  u=as.matrix(u)
  #Zs=Zs[,i]
  X=X[,i]
  res=X %*% u
...
fwrite(pgs,sprintf('pgs/Rep%.0f_DTS_FT_PGS.txt',rep),row.names=F,quote=F,sep='\t')
