...
for a in ${threshs[@]}; do
  for i in {1..57}; do
    ...
    #Haplotype threshold
    Rscript GridLMM_haplotypes/permute_haplo/find_threshold.R $pheno $env $a
    wait
    #Founder threshold
    Rscript GridLMM_founderprobs/permute/find_threshold.R $pheno $env $a
    wait
    #600K threshold
    Rscript GridLMM_600KSNP/permute/find_threshold.R $pheno $env $a
    wait
  done
done
