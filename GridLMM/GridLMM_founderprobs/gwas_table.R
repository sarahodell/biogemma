...
for(i in seq(1,10)){
      tmp_gwas=c()
      for(p in phenotypes){
      	    g=readRDS(sprintf('models/Biogemma_chr%.0f_%s_x_%s_founderprobs.rds',i,p,env))
...
fwrite(full_gwas,sprintf('Founder_GWAS_%s_results.txt',env),row.names=F,quote=F,sep='\t')