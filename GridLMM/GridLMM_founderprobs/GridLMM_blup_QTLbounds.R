...
cutoff=threshtable[threshtable$phenotype==pheno & threshtable$method=="founder_probs" & threshtable$environment=="ALL",]$threshold
...
tmp=lapply(X_list_ordered, function(x) x[data_blup$ID,location])
...
fwrite(chrom,sprintf('bound_images/%s_ALL_founderprobs_chr%s_LRT.txt',pheno,chr),row.names=F,quote=F)

png(sprintf('bound_images/%s_ALL_founderprobs_chr%s_LRT.png',pheno,chr),width=800,height=800)
...
log10(p-value)") + ggtitle(sprintf("%s BLUP Founder Prob LRT Chr %s",pheno,chr)) + theme_classic())
dev.off()
...
#plot(-log10(pchisq(gwas_marker$ML_logLik - gwas_null_drop$ML_logLik,1,lower.tail=F)))
