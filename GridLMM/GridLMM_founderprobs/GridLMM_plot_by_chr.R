...
for(i in 1:10){
   pmap=fread(sprintf('../genotypes/qtl2/startfiles/Biogemma_pmap_c%.0f.csv',i),data.table=F,stringsAsFactors=F)
   mod=readRDS(sprintf('GridLMM_founderprobs/models/Biogemma_chr%.0f_%s_x_%s_founderprobs.rds',i,pheno,env))
...
png(sprintf('GridLMM_founderprobs/images/%s_x_%s_manhattan.png',pheno,env),width=960,height=680)
