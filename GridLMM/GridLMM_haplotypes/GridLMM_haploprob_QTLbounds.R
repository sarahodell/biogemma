...
cutoff=threshtable[threshtable$phenotype==pheno & threshtable$method=="haplotype_probs" & threshtable$environment==env,]$threshold
...
fwrite(all_chrom,sprintf('bound_images/%s_%s_haplotypeprobs_chr%s_LRT.txt',pheno,env,chr),row.names=F,quote=F)

png(sprintf('bound_images/%s_%s_haplotypeprobs_chr%s_LRT.png',pheno,env,chr),width=800,height=800)
print(ggplot(all_chrom,aes(x=pos,y=lrt)) + geom_point(aes(color=sig),alpha=0.5) + scale_color_manual(breaks=chrom$sig,values=c("FALSE"="black","TRUE"="red")) + geom_hline(yintercept=cutoff,color="green") + geom_hline(yintercept=(-log10(min_pval)),color="blue") + guides(color=F) + xlab("Position (Mb)") + ylab("-log10(p-value)") + ggtitle(sprintf("%s %s Haplotype Prob LRT Chr %s",pheno,env,chr)) + theme_classic())
dev.off()

png(sprintf('bound_images/%s_%s_haplotypeprobs_chr%s_LRT_by_hapgrp.png',pheno,env,chr),width=800,height=800)
print(ggplot(all_chrom,aes(x=pos,y=lrt)) + geom_point(aes(color=as.factor(hapgrp)),alpha=0.5) + geom_hline(yintercept=cutoff,color="green") + geom_hline(yintercept=(-log10(min_pval)),color="blue") + xlab("Position (Mb)") + ylab("-log10(p-value)") + ggtitle(sprintf("%s %s Haplotype Prob LRT Chr %s",pheno,env,chr)) + theme_classic())
dev.off()
