...
#geno=fread(sprintf('../../genotypes/qtl2/Biogemma_DHgenos/bg%s_filtered_600K.csv',chr),data.table=F,stringsAsFactors=F)
...
#X=as.data.frame(X)
rownames(X)=X$ind
X=X[,colnames(X)[2:dim(X)[2]]]
#colnames(X)=colnames(X)[2:dim(X)[2]]
